...
class Comment {
  ...
  static async create({ user_id, resource_id, text }) {
    const sql = `
      ...
      RETURNING *;
    `;
    const { rows } = await db.query(sql, [user_id, resource_id, text]);
    return rows[0];
  }

  // 2. Get Comments for a specific Resource
  static async findByResourceId(resource_id) {
    const sql = `
      SELECT comments.id, comments.text, comments.created_at, users.name as user_name
      FROM comments
